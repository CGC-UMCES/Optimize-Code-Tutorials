...
depths = np.full((n_cells, n_layers, n_days), np.nan)
nwcbox_vals = np.full((n_cells, n_layers, n_days), np.nan)
...
def process_nwcbox(jday, nwcbox_val):
    ...
    return cell_id, layernumber, jday, computed_values, lon, lat, Depth, nwcbox_val, c1_array_nwcbox
...
for jday in range(start_jday, end_jday + 1):
    ...
    for result in results:
        cell_id, layernumber, jday, computed_values, lon, lat, Depth, nwcbox_val, c1_array_nwcbox = result
        ...
        depths[cell_id - 1, layernumber - 1, jday - start_jday] = Depth
        nwcbox_vals[cell_id - 1, layernumber - 1, jday - start_jday] = nwcbox_val

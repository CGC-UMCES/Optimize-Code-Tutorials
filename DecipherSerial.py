...
print_memory_usage("After loading configuration")
...
print_memory_usage("After reading binary file data")
...
print_memory_usage("After initializing NetCDF file")
...
for jday in range(start_jday, end_jday + 1):
    ...
    print_memory_usage(f"After reading binary data for jday {jday}")
    c1_array = data_real['c1'].reshape(NAC, nwcbox)

    # Read CSV data
    df = pd.read_csv(csv_file)
    print_memory_usage("After reading CSV data")
    ...
    for nwcbox_val in range(1, nwcbox + 1):
        ...
        if nwcbox_val % 10000 == 0:
            elapsed_time_l = time.time() - start_time_l
            print(f"Processed # of nwcbox: {nwcbox_val}, Time taken for computing: {elapsed_time_l*nwcbox_val:.2f} seconds")
        ...
        if nwcbox_val % 10000 == 0:
            elapsed_time_l = time.time() - start_time_l
            print(f"Processed # of nwcbox: {nwcbox_val}, Time taken for computing + saving to netcdf: {elapsed_time_l*nwcbox_val:.2f} seconds")
            print_memory_usage(f"After processing nwcbox {nwcbox_val}")
...
print_memory_usage("After closing NetCDF file")
